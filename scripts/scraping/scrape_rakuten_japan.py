"""
Rakuten Japan scraper with OBF-seeded target expansion.
"""
...
source_config = load_scraping_source_config(root, "rakuten_japan")
...
auto_seed_max_pairs = int(source_config.get("auto_seed_max_pairs", 220))
...
def search_rakuten(query: str, session: Any) -> str | None:
    """
    Search Rakuten Japan and return first item URL.
    """
...
def extract_price_rakuten(soup: Any) -> tuple[float | None, float | None, bool]:
    """
    Extract current and original prices from Rakuten item page.
    """

    text = soup.get_text(" ", strip=True)
    amounts = re.findall(r"([0-9][0-9,]*)\s*円", text)
    values: list[float] = []
    for raw in amounts:
        try:
...
def scrape_product(
    ...
    if not url:
        ...
        url = search_rakuten(query, session) or ""
    ...
    try:
        ...
        soup = beautiful_soup(resp.text, "html.parser")
        price, original_price, on_promotion = extract_price_rakuten(soup)
        if price is None:
...
def main(
    ...
    results: list[dict] = []
    found_urls: list[str] = []
    log.info(f"Rakuten Japan scrape — {len(seed_targets)} products {'[DRY RUN]' if dry_run else ''}")
